...
export default function NotificationBell({ userId, userEmail }) {
  ...
  useEffect(() => {
    const handler = (e) => { if (ref.current && !ref.current.contains(e.target)) { setOpen(false); queryClient.invalidateQueries({ queryKey: ['notifications', userId] }); } };
        ...
        <div className="absolute right-0 top-12 w-80 bg-card border border-border rounded-2xl z-50 overflow-hidden">
          ...
          <div className="flex items-center justify-between px-4 py-3 border-b border-border">
            ...
            <div className="flex items-center gap-2">
              ...
              <button onClick={() => { setOpen(false); queryClient.invalidateQueries({ queryKey: ['notifications', userId] }); }} className="text-muted-foreground hover:text-foreground">
                <X className="w-4 h-4" />
              </button>
            </div>
          </div>
